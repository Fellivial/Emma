...
| `greeting-engine.ts`      | Session-aware greetings: time of day, absence length (quick return → very long absence), memory-enriched follow-ups                                                                    |
| `proactive-speech.ts`     | Unprompted speech on idle (45s comment, 2min concern, late-night nudge), memory-personalized                                                                                           |
| `pattern-detector.ts`     | Daily cron: detects recurring task patterns, generates in-persona scheduling suggestions                                                                                               |
...
`src/proxy.ts` gates all routes via Supabase SSR. Public paths: `/login`, `/register`, `/auth/callback`, `/landing`, `/api/waitlist`, `/api/emma/webhook`, `/waitlist`, and `/api/emma/unsubscribe`. API routes authenticate inside each handler. When `NEXT_PUBLIC_SUPABASE_URL` is not set (local dev), middleware is a no-op.
...
| `EMMA_ENCRYPTION_KEY_PREVIOUS`              | Optional — retiring key during rotation; reads fall back to it, writes never use it. Migrate with `scripts/rotate-encryption-key.ts`, then remove                      |
...
| `RESEND_API_KEY`                            | Email sequences and invite notifications                                                                                                                               |
...
| `HUBSPOT_CLIENT_ID`                         | HubSpot OAuth 2.0 Client ID — required for token refresh (`src/lib/oauth-refresh.ts`)                                                                                  |
| `HUBSPOT_CLIENT_SECRET`                     | HubSpot OAuth 2.0 Client Secret — required for token refresh                                                                                                           |
