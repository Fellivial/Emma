...
function InputValue({ tool, k, v }: { tool: string; k: string; v: string }) {
  if (tool === "send_email") {
    ...
    if (k === "subject")
      return (
        <span style={{ fontStyle: "italic", color: "rgba(244,193,221,0.5)", fontSize: 11 }}>
          {v}
        </span>
      );
    if (k === "body")
    ...
    <div data-approval-id={approvalId} style={{ display: "flex", gap: 8, padding: "4px 0" }}>
      ...
      <div
        style={{
          ...
          borderRadius: "50%",
          flexShrink: 0,
          background: "linear-gradient(135deg,rgba(232,160,191,0.3),rgba(168,93,154,0.2))",
      ...
      >
        ...
        >
          <span style={{ fontSize: 11, fontWeight: 500, color: "rgba(217,119,6,0.8)" }}>
            ⚠ Action Required
          </span>
          <span style={{ fontSize: 10, color: "rgba(244,193,221,0.3)", fontFamily: "monospace" }}>
            {timeLeft}
          </span>
        </div>
          ...
          {Object.entries(inputs).map(([k, v]) => (
            <div
              ...
              style={{
                ...
                borderBottom: "1px solid rgba(232,160,191,0.06)",
                padding: "4px 0",
              }}
            >
              <span
