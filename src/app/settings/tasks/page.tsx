...
import { Check, X, AlertTriangle, TrendingUp, ChevronRight, Repeat } from "lucide-react";
...
export default function TasksPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-8">
      <div className="mb-6">
        <h1 className="text-xl font-light text-emma-100">Tasks</h1>
        <p className="text-xs text-emma-300/50 mt-1">
          Approvals, history, and autonomous action log.
        </p>
      </div>
      {/* Free-plan gate */}
      {!loading && planId === "free" && (
        <div className="rounded-2xl border border-emma-300/15 bg-emma-300/3 p-8 flex flex-col items-center text-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-sm font-medium text-emma-200/70 mb-1">
              Autonomous mode is a Starter feature
            </h2>
            <p className="text-xs font-light text-emma-200/30 max-w-sm">
              Upgrade to Starter to unlock scheduled tasks, webhooks, and up to 3 autonomous actions
              per hour.
            </p>
          </div>
          <a
            ...
            <div className="flex flex-col gap-3">
              {approvals.length === 0 ? (
                ...
                approvals.map((a) => (
                  <div
                    key={a.id}
                    className="rounded-xl border border-amber-400/15 bg-amber-400/3 p-4"
                  >
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <ChevronRight
                          size={12}
                          className="text-emma-200/15 group-hover:text-emma-300/40 transition-colors"
                        />
                      </div>
                    </div>
            ...
            <div className="flex flex-col gap-1">
              {actions.length === 0 ? (
                ...
                actions.map((a) => (
                  <div
                    key={a.id}
                    className="flex items-center gap-3 py-2.5 px-3 rounded-lg hover:bg-surface transition-colors"
                  >
                    <StatusDot status={a.status} />
                    <span className="text-xs font-mono text-emma-200/40 w-32 shrink-0">
                      {a.action}
                    </span>
                    <span className="text-[11px] text-emma-200/25 flex-1 truncate">
                      {a.task_id}
                    </span>
                    <span className="text-[10px] text-emma-200/15 w-16 text-right">
                      {formatTokens(a.token_cost)}
                    </span>
                    <span className="text-[10px] text-emma-200/10 w-28 text-right">
                      {formatTime(a.created_at)}
                    </span>
                  </div>
                ))
      ...
      {patterns.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-medium text-emma-300 tracking-wider">
              Detected Patterns
            </span>
          </div>
          <div className="flex flex-col gap-3">
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-emma-200/40 font-light truncate mb-1">
                      {t.goal}
                    </p>
                    <p className="text-xs text-emma-200/60 font-light line-clamp-2">
                      {summaries.get(t.id)}
                    </p>
                  </div>
...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${colors[status] || "bg-emma-200/20"}`} />
  );
}
