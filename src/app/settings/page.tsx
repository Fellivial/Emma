    ...
    <div className="max-w-3xl mx-auto px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-xs text-emma-300/50 mt-1">
          Persona configuration, voice, and behaviour preferences.
        </p>
      </div>
      {/* Quick usage bar */}
      <div className="rounded-xl border border-surface-border bg-surface p-4 mb-6 grid grid-cols-4 gap-4">
        <Stat
          label="Today"
          value={`${usage.dailyMessages} msgs`}
          sub={`/ ${config.messageLimitDaily}`}
        />
        <Stat label="Today Tokens" value={formatTokens(usage.dailyTokens)} />
        <Stat
          label="This Month"
          value={formatTokens(usage.monthlyTokens)}
          sub={`/ ${formatTokens(config.tokenBudgetMonthly)}`}
        />
      ...
      <Section title="Persona">
        ...
        <Field
          ...
          sub="Custom personality prompt (leave empty for default Mommy)"
        >
          <textarea
      ...
      <Section title="Voice">
        <Field
          ...
          sub="Override the default voice (leave empty for Rachel)"
        >
          <input
      ...
      <div className="flex items-center gap-3 mt-8">
        ...
        {saved && (
          <span className="text-xs text-emerald-400/60">
            ✓ Saved — changes apply on next session
          </span>
        )}
      </div>
