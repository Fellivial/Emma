...
import { ArrowLeft, DollarSign, Users, Zap, TrendingUp, BarChart3, Gift, Database } from "lucide-react";

interface Client {
  id: string; slug: string; name: string; plan: string; memberCount: number;
  tokenBudget: number; monthlyTokens: number; monthlyMessages: number;
  budgetUsed: number; estimatedCost: number; createdAt: string;
}

interface Overview {
  mrr: number; mrrPrev: number; mrrGrowth: number;
  totalClients: number; paidClients: number; freeClients: number;
  churnRate: number; waitlistCount: number;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emma-950 via-emma-900 to-emma-950 font-sans">
        <div className="text-center">
          ...
          <Link href="/" className="text-xs text-emma-300/40 mt-4 block hover:text-emma-300">← Back</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-5 gap-3 mb-6">
            <MetricCard icon={<DollarSign size={14} />} label="MRR" value={`$${overview?.mrr?.toLocaleString() || 0}`}
              ...
              highlight />
            <MetricCard icon={<Users size={14} />} label="Paid Clients" value={String(overview?.paidClients || 0)}
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            ...
            <div className="rounded-xl border border-surface-border bg-surface p-5">
              <h3 className="text-xs font-medium text-emma-200/30 uppercase tracking-widest mb-3">Top Channels</h3>
              {channels.length === 0 ? (
                <div className="text-[11px] text-emma-200/15 py-4 text-center">No channel data yet</div>
              ) : (
                  ...
                  {channels.slice(0, 6).map((ch) => {
                    ...
                    return (
                      <div key={ch.channel} className="flex items-center gap-3">
                        <span className="text-[11px] text-emma-200/50 w-20 truncate">{ch.channel}</span>
                        <div className="flex-1 h-2 rounded-full bg-emma-200/5 overflow-hidden">
                          <div className="h-full rounded-full bg-emma-300/40" style={{ width: `${(ch.signups / maxSignups) * 100}%` }} />
                        </div>
                        <span className="text-[11px] text-emma-200/30 w-8 text-right">{ch.signups}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="rounded-xl border border-surface-border bg-surface p-5">
              <h3 className="text-xs font-medium text-emma-200/30 uppercase tracking-widest mb-3">Referrals & Affiliates</h3>
              ...
              {affiliates.totalCommissions > 0 && (
                <div className="text-[10px] text-emma-200/20 border-t border-surface-border pt-2 mt-1">
                  Total commissions paid: <span className="text-amber-300/50">${affiliates.totalCommissions.toFixed(2)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-surface-border bg-surface p-5 mb-6">
            <h3 className="text-xs font-medium text-emma-200/30 uppercase tracking-widest mb-3">Plan Distribution</h3>
            <div className="flex items-end gap-2 h-24">
              {["Free", "Starter", "Pro", "Enterprise"].map((plan) => {
                const count = planDist[plan] || 0;
                const max = Math.max(...Object.values(planDist), 1);
                const colors: Record<string, string> = {
                  Free: "bg-emma-200/15", Starter: "bg-emerald-400/40", Pro: "bg-blue-400/40", Enterprise: "bg-emma-300/50",
                };
                return (
                  <div key={plan} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-xs text-emma-200/40">{count}</span>
                    <div className="w-full rounded-t-md" style={{ height: `${Math.max(4, (count / max) * 80)}px` }}>
                      <div className={`w-full h-full rounded-t-md ${colors[plan] || "bg-emma-200/10"}`} />
                    </div>
          ...
          {/* ── Row 4: Client Table ───────────────────────────────────────── */}
          <h3 className="text-xs font-medium text-emma-200/30 uppercase tracking-widest mb-3">All Clients</h3>
              ...
              <tbody>
                {clients.map((c) => (
                  <tr key={c.id} className="border-b border-surface-border/50 hover:bg-surface/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right text-emma-200/40 font-mono">{fmtTokens(c.monthlyTokens)}</td>
                    <td className="px-4 py-3 text-right"><BudgetBar used={c.budgetUsed} /></td>
                    <td className="px-4 py-3 text-right text-emma-200/40">${c.estimatedCost}</td>
                    <td className="px-4 py-3 text-right text-emma-200/25">{new Date(c.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function MetricCard({ icon, label, value, sub, highlight, warn }: {
  icon: React.ReactNode; label: string; value: string; sub?: string; highlight?: boolean; warn?: boolean;
}) {
  return (
    <div className={`rounded-xl border p-4 ${
      highlight ? "border-emerald-400/20 bg-emerald-400/3"
        : warn ? "border-red-400/15 bg-red-400/3"
        : "border-surface-border bg-surface"
    }`}>
      <div className={`mb-1.5 ${highlight ? "text-emerald-300/50" : warn ? "text-red-300/50" : "text-emma-200/20"}`}>{icon}</div>
      <div className={`text-xl font-light ${highlight ? "text-emerald-300" : warn ? "text-red-300" : "text-emma-200/60"}`}>{value}</div>
  ...
  return (
    <div>
      <div className="flex justify-between text-[11px] mb-1">
        ...
        <span className="text-emma-200/30">{value} ({pct}%)</span>
      </div>
      <div className="h-2 rounded-full bg-emma-200/5 overflow-hidden">
        <div className={`h-full rounded-full ${color === "emerald" ? "bg-emerald-400/50" : "bg-emma-300/30"}`}
...
function PlanBadge({ plan }: { plan: string }) {
  ...
  return <span className={`text-[10px] px-2 py-0.5 rounded-full border ${colors[plan] || colors.Free}`}>{plan}</span>;
}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-16 h-1.5 rounded-full bg-emma-200/5 overflow-hidden">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${Math.min(100, used)}%` }} />
      </div>
