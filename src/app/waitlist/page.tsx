...
export default function WaitlistPage() {
  const [spots, setSpots] = useState<SpotData>({ spotsRemaining: 10, totalSpots: 10, waitlistCount: 0 });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emma-950 via-emma-900 to-emma-950 font-sans text-emma-100 flex items-center justify-center p-6">
        ...
        {result ? (
          <div className={`rounded-2xl border p-8 text-center ${
            ...
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              ...
              <Check size={24} className={result.result === "accepted" ? "text-emerald-300" : "text-emma-300"} />
            </div>

            {result.result === "accepted" ? (
              <>
                <h2 className="text-lg font-light text-emma-200/80 mb-2">You're in.</h2>
                <p className="text-sm font-light text-emma-200/40 mb-1">Emma will reach out shortly.</p>
              ...
              <>
                ...
                <p className="text-xs text-emma-200/20">We'll contact you personally when a spot opens.</p>
              </>
            ) : (
              <>
            ...
            <div className="text-center mb-8">
              {spotsFilled ? (
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-400/8 border border-amber-400/15">
                  ...
                  <span className="text-sm font-light text-amber-300/70">All spots filled — join the waitlist</span>
                </div>
              ) : (
                <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-emerald-400/5 border border-emerald-400/15">
                  ...
                  <span className="text-2xl font-light text-emerald-300">{spots.spotsRemaining}</span>
                  <span className="text-sm font-light text-emma-200/30">of {spots.totalSpots} spots remaining</span>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-surface-border bg-surface p-8">
              <div className="mb-6">
                <p className="text-sm font-light text-emma-200/50 italic mb-3">
                  "Hey! I'm Emma. I'm only taking {spots.totalSpots} people right now — want one of the spots?"
                </p>
                ...
                <p className="text-xs font-light text-emma-200/25 leading-relaxed">
                  Emma is an AI agent that sees, speaks, remembers, and acts — built for one specific problem in your business.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] text-emma-200/25 uppercase tracking-widest block mb-1">Name *</label>
                    <input value={name} onChange={(e) => setName(e.target.value)}
                      placeholder="Your name" className="wl-input" />
                  </div>
                  <div>
                    <label className="text-[10px] text-emma-200/25 uppercase tracking-widest block mb-1">Email *</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                      placeholder="you@company.com" className="wl-input" />
                  </div>
                </div>

                <div>
                  <label className="text-[10px] text-emma-200/25 uppercase tracking-widest block mb-1">Industry *</label>
                  <select value={industry} onChange={(e) => setIndustry(e.target.value)} className="wl-input appearance-none">
                ...
                <div>
                  <label className="text-[10px] text-emma-200/25 uppercase tracking-widest block mb-1">What problem do you want Emma to solve?</label>
                  <textarea value={message} onChange={(e) => setMessage(e.target.value)}
                    placeholder="2–3 sentences about your use case" rows={3} className="wl-input resize-y" />
                </div>

                <div>
                  <label className="text-[10px] text-emma-200/25 uppercase tracking-widest block mb-1">How did you hear about Emma?</label>
                  <input value={referralSource} onChange={(e) => setReferralSource(e.target.value)}
                    placeholder="LinkedIn, friend, podcast…" className="wl-input" />
                </div>
      ...
      <style jsx>{`
        ...
        .wl-input:focus { border-color: rgba(232,160,191,0.2); }
        .wl-input::placeholder { color: rgba(232,160,191,0.12); }
        select.wl-input { color: rgba(232,160,191,0.5); }
        select.wl-input option { background: #1a1018; color: #e8dfe6; }
      `}</style>
    </div>
  );
}
