...
- **Real verification.** Phase 3 implemented this for Storage only — `StorageBucketAdapter.verify()` now re-lists the user's folder for real, replacing the Phase 2 stub. Every database resource's `verificationAdapter` is still `null` (Registry-driven `"skipped"` entries in `verify_database`/`verify_external`) — real per-table database verification remains unbuilt, deferred to whichever future phase implements it, not add a parallel verification mechanism outside the existing adapter lifecycle.
