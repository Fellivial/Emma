...
- Independent Review: [`docs/phase0-independent-review.md`](phase0-independent-review.md) — recommendation "Approved with Minor Revisions"; the revisions below (§7–§9, and the corrections in §1.1/§1.2/§1.5/§4) were applied to this document as a direct result of that review

This single document contains all six Phase 0 deliverables as sections, plus three sections (§7–§9) added after the Independent Review to close gaps it identified:
...
7. Boundary Inventory (§7 — added post-Independent-Review)
8. Component Ownership Map (§8 — added post-Independent-Review)
9. Phase 0 Conclusion (§9 — added post-Independent-Review)

The roadmap's own Phase 0 deliverable list (`roadmaps/brain-gateway-roadmap-v1.md`, Phase 0 Deliverables) names five differently-titled items (Existing Architecture Inventory, Dependency Inventory, Current AI Flow, Documentation Review Report, Initial Findings); the six-way split above is a re-mapping of that same content (architecture is split into a diagram, §2, and a flow diagram, §3) rather than a scope deviation. §7–§9 are additions beyond the roadmap's named list, added because the Independent Review identified them as missing information Phase 1 will need.
...
Emma's inference boundary is not greenfield. A Brain Gateway already exists and is load-bearing in production: `src/core/brain/gateway.ts` is the sole entry point for `brainChat` / `brainChatStream` / `brainEmbed`, backed by a single OpenRouter provider implementation (`src/core/brain/providers/openrouter.ts`), consumed by 14 non-test source files (16 invocation sites — two files call it twice) across API routes and core modules, none of which touch provider wire details directly. This was designed under ADR-0003 and implemented under Phase 7B (2026-07-14), documented in `docs/phase7a-brain-architecture-readiness.md` and `docs/phase7b-brain-gateway-implementation-report.md`.
...
| Request/body schema validation     | `src/core/request-validation.ts` (used by `history/route.ts`, `agent/route.ts`; **not** imported by the main chat route, `route.ts` — added per Independent Review, §1.4 step 4) |
...
| API routes (9 files: `route.ts`, `vision`, `emotion`, `summarize`, `history`, `persona`, `memory`, `ingest/whatsapp`, `cron/reflection`) + core/lib modules (5 files: `embeddings.ts`, `agent-loop.ts`, `tool-registry.ts`, `task-summarizer.ts`, `pattern-detector.ts`) — 14 total, 16 invocation sites | `src/core/brain/gateway.ts`                                                                                                                                                                                       | Direct import of `brainChat`/`brainChatStream`/`brainEmbed`/`isBrainConfigured` — never import `providers/openrouter.ts` or `types.ts` directly. (Corrected per Independent Review: the original "15 API routes" figure both miscounted and mis-categorized this group — see §7.) |
...
Added after the [Independent Review](phase0-independent-review.md) identified that boundary information, while accurate, was scattered across §1.2/§1.3/§4 rather than consolidated into one artifact. This section consolidates that existing understanding — it introduces no new investigation or architectural decision, only a restatement of what §1–§4 already establish, in a single "owns / must not do" table per component, following the pattern ADR-0003 already uses for the coarser Application/Gateway/Provider split.

| Component                                                          | Owns / responsible for                                                                                                                                     | Must not do (per current implementation)                                                                                                                                                                                          |
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UI (`page.tsx`, client engines)                                    | Rendering, user input capture, client-side context trimming, orchestrating TTS/avatar playback after a response                                            | Never imports `src/core/brain/*` or any provider file directly; reaches inference only via `fetch()` to `/api/emma*`                                                                                                              |
| API (`/api/emma/route.ts` and sibling routes)                      | Auth, rate limiting, cost gating, composing all AI-pipeline inputs (memory/behavior/prompt), calling the gateway, assembling the SSE response              | Does not itself construct provider requests or know provider wire shapes — delegates entirely to the Brain Gateway                                                                                                                |
| Brain Gateway (`src/core/brain/gateway.ts`)                        | Sole entry point for `brainChat`/`brainChatStream`/`brainEmbed`; normalizes provider responses into `BrainMessage`/`BrainFinishReason`/`BrainRequestError` | Does not itself know OpenRouter's wire format — delegates to the provider layer; does not perform routing/model-selection beyond a fixed task-tier                                                                                |
| Provider (`src/core/brain/providers/openrouter.ts`)                | The only file permitted to know OpenRouter's URL, headers, request/response shape, SSE parsing, and status-code normalization                              | Not imported by anything except `gateway.ts`; must not be reached by API routes, UI, or other core modules directly (confirmed: it isn't)                                                                                         |
| Memory (`src/core/memory-db.ts`)                                   | Retrieval (keyword-heuristic scoring), persistence (encrypted), capacity enforcement, supersede-on-change                                                  | Does not decide behavior/tone; does not talk to the LLM provider                                                                                                                                                                  |
| Behavior (`src/core/behavior-flags.ts`)                            | Deterministic derivation of `BehaviorFlags` from persona/memory/emotion/time                                                                               | Does not render prompt text itself (that's the Prompt Builder's job) and does not call the LLM                                                                                                                                    |
| Emotion (`src/core/emotion-engine.ts`)                             | Client-side signal capture/fusion (voice/vision/text) into `EmotionState`                                                                                  | Its raw output is consumed by exactly two things (Behavior, Prompt Builder) — avatar/voice/greeting/proactive-speech must consume derived `BehaviorFlags` instead, never raw emotion, per the existing ADR-0001 design constraint |
| Prompt Builder (`src/core/personas.ts`)                            | Formatting persona + memories + context + behavior directives into the system prompt string                                                                | Does not fetch its own inputs (route.ts is the composition root) and does not derive behavior itself                                                                                                                              |
| Response Processing (`command-parser.ts`, `response-validator.ts`) | Extracting control tags from raw model output; log-only post-hoc behavior-flag compliance check                                                            | Does not rewrite or block the response — validation is observability-only by explicit design                                                                                                                                      |
| TTS (`tts/route.ts`, `voice-behavior.ts`)                          | Direct ElevenLabs integration; behavior-flag-driven prosody modulation                                                                                     | Explicitly outside the Brain Gateway's scope (ADR-0003); does not participate in LLM inference                                                                                                                                    |
| Avatar (`avatar-engine.ts`)                                        | Live2D expression/animation/lip-sync, tap-reaction and idle-cadence gating by behavior flags                                                               | Does not import the Brain Gateway, memory, or emotion modules directly — only consumes `event.expression` (via the UI) and `BehaviorFlags`                                                                                        |

## 8. Component Ownership Map

Added for the same reason as §7 — consolidating, not re-investigating, ownership information already present in §1.2, §1.3, and §1.7.

| Responsibility                 | Single owner                                                                                                                          | Known exceptions / splits (see §1.7 for detail)                                                                                                                                                         |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prompt construction            | `src/core/personas.ts` (`buildSystemPrompt`)                                                                                          | 5 independent hardcoded system prompts exist outside it: `vision/route.ts`, `summarize/route.ts`, `ingest/whatsapp/route.ts`, and two in `history/route.ts`                                             |
| Behavior derivation            | `src/core/behavior-flags.ts` (`deriveBehaviorFlags`)                                                                                  | Invoked independently in 3 places (server `route.ts`, client `page.tsx`, `agent-loop.ts`'s notification path) with differing input completeness                                                         |
| Memory retrieval               | `src/core/memory-db.ts` (`getRelevantMemoriesForUser`)                                                                                | None — single owner, no split                                                                                                                                                                           |
| Routing / model selection      | No routing exists; model IDs in `src/core/models.ts`, tier-to-fallback-array mapping in `providers/openrouter.ts`                     | Embedding model id is hardcoded in the provider file rather than `models.ts` — a minor split of "the model registry" across two files                                                                   |
| Response validation            | `src/core/response-validator.ts` (`validateResponseBehavior`)                                                                         | Covers only 3 of 5 behavior flags (`warmth`/`initiative` unchecked)                                                                                                                                     |
| Provider communication         | `src/core/brain/providers/openrouter.ts`                                                                                              | TTS (`tts/route.ts`) and STT (`stt/route.ts`) each communicate with their own provider directly, outside this owner's scope, by explicit design                                                         |
| Request/body schema validation | `src/core/request-validation.ts`                                                                                                      | Used by `history/route.ts` and `agent/route.ts`; **not** used by the main chat route (`route.ts`), which parses its body with no runtime schema validation                                              |
| Rate limiting                  | Split: `src/lib/ratelimit.ts` (distributed, per-request) and `src/core/rate-limiter.ts` (Supabase-backed, agent-task hourly counters) | Two independent implementations; the chat route additionally applies both a direct distributed-limiter check and a cost-gate check that reuses the same distributed limiter under a different namespace |

## 9. Phase 0 Conclusion

The current AI architecture has been reviewed to the depth required by the roadmap's Phase 0 exit criteria: all 11 scoped subsystems (request flow, provider integration, prompt construction, context management, memory, behavior, emotion, response processing, TTS, avatar, configuration) have been traced to concrete file/line evidence; existing documentation has been inventoried and cross-checked against the code; dependency directions and component ownership have been documented (§4, §7, §8); and technical debt has been catalogued as observations only (§1.7, §6). This understanding was independently reviewed (see [`docs/phase0-independent-review.md`](phase0-independent-review.md)), which found no Critical or Major defect and confirmed the document's central and headline claims by direct re-verification. On that basis, the current architecture is considered sufficiently understood to serve as the foundation for Phase 1 — Brain Gateway Architecture Review, pending the user's approval and merge of this pull request.
...
- [x] All six original deliverables completed (§1–§6)
- [x] No implementation changes introduced (documentation-only branch)
- [x] Independent review completed with no Critical/Major findings; recommended revisions applied (§7–§9, and corrections in §1.1/§1.2/§1.5/§4) — see [`docs/phase0-independent-review.md`](phase0-independent-review.md)
