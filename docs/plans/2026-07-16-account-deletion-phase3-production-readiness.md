...
3. **Live validation (Phase 3.1, Task 2) found the real `deleteUserOwnedData()` RPC call cannot currently complete successfully on the linked "Emma" Supabase project** — it is missing `document_chunks.user_id` (confirmed directly: the RPC call itself errors `unknown column: document_chunks.user_id`), so every real deletion request against this environment retries 3 times and then permanently fails at `deleting_database`. `personas`/`push_subscriptions`/`proactive_daily` were not individually re-tested this way — the shared trait these four tables have, per this document's own "Phase 2.1: what live validation actually found" section, is that their definitions exist only in `schema.sql`, never as a tracked migration, which is a migration-tracking gap category, not a confirmed shared column defect. This was not fixed here — touching the live schema was explicitly out of scope for this phase — but the `document_chunks` finding itself is a real, live-verified result, not a hypothetical one. See the Phase 3.1 Live Production Validation Report for the full evidence log. Whether production has the same gap (for `document_chunks` or any of the other three) is unknown and needs separate confirmation before this workflow can be considered safe to rely on there.
